...
from networks.EDFIDepth import EDFIDepth
from logutil import *
from excelutils import *

parser = argparse.ArgumentParser(description='MPViT-EDFIDepth PyTorch implementation.', fromfile_prefix_chars='@')
parser.convert_arg_line_to_args = convert_arg_line_to_args

parser.add_argument('--mode', type=str, help='train or test', default='train')
parser.add_argument('--model_name', type=str, help='model name', default='mpvit_edfidepth')

parser.add_argument('--encoder', type=str, help='type of encoder, base07, large07', default='large07')
parser.add_argument('--pretrain', type=str, help='path of pretrained encoder', default=None)
# parser.add_argument('--pretrain_res',                  type=str,   help='path of pretrained encoder', default=None)
# Dataset
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = EDFIDepth(version=args.encoder, inv_depth=False, max_depth=args.max_depth, pretrained=args.pretrain)
    model.train()
    ...
    # 初始化记录表
    result_file_name = initial_excel_file(args.batch_size, args.dataset, ngpus_per_node, args.model_name)
    print("== Initial excel file:" + result_file_name)
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            if not args.multiprocessing_distributed or (
                ...
                progress_bar(step % args.eval_freq, args.eval_freq,
                             ('Eval Frequncy: %d, epoch: %d / %d, global_step: %d ,lr: %.12f, loss: %.12f') % (
                             args.eval_freq,
                             epoch+1, args.num_epochs, global_step, current_lr, loss))
            ...
            if args.do_online_eval and global_step and global_step % args.eval_freq == 0 and not model_just_loaded:
                ...
                if eval_measures is not None:
                    # 记录到excel表中
                    log_to_new_excel(result_file_name, step % args.eval_freq, args.eval_freq, global_step, epoch + 1, eval_measures)
                    print("== Already write evaluation results to excel file:" + result_file_name)
